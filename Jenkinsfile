pipeline {
    ...
    environment {
        ...
        CODECOV_TOKEN = credentials('codecov-provider-nop')
    }
            ...
            steps {
                script {
                    if (env.CHANGE_ID != null) {
                        def json = sh (script: "curl -s https://api.github.com/repos/crossplane/provider-nop/pulls/${env.CHANGE_ID}", returnStdout: true).trim()
